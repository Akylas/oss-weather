	...
	<gridlayout rows="auto,*">
		...
		{:else}
			<gridlayout id="teststack" row="1" rows="auto,auto,auto,auto,60" horizontalAlignment="center" verticalAlignment="center" columns="auto">
				...
				<label id="teststacklabel" fontSize="24" textAlignment="center" text={l('no_location_setup')} color="white" />
				<button row="1" margin="4 0 4 0">
				...
				<button row="2" margin="4 0 4 0">
                 ...
                 <button row="3" on:tap={ () => show = !show } text='show' margin="4 0 4 0"/>
                {#if show}
                    <label row="4" transition:fade text='hi there' />
                {/if}
				<!-- <button variant="text" horizontalAlignment="center">
					<span class="mdi" text="mdi-magnify" />
					<span text=" search location" />13
				</button> -->
			</gridlayout>
		{/if}
	</gridlayout>
