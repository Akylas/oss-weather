...
function getDaily(weatherLocation: WeatherLocation, hourly: Hourly[], hourlyForecast: ForecastForecast[], dailyForecast: Dailyforecast) {
...
export async function getWeather(weatherLocation: WeatherLocation) {
    ...
    const result = await Promise.all([fetch<MFForecastResult>('v2/forecast', coords), fetch<MFMinutely>('v3/nowcast/rain', coords), fetch<MFCurrent>('v2/observation', coords)]);
    ...
    if (forecast.properties.french_department) {
        warnings = await fetch<MFWarnings>('v3/warning/full', {
            ...coords,
    ...
    const hourlyData = forecast.properties.forecast?.slice(0, hourlyLastIndex).map((data, index) => {
        ...
        d.icon = convertMFICon(data.weather_icon);
    ...
    const r = {
        currently: current
            ? weatherDataIconColors(
                  {
                      ...
                      icon: convertMFICon(current.properties.gridded.weather_icon),
            ...
            : {},
        daily: {
            data: forecast.properties.daily_forecast.slice(0, 14).map((data) => getDaily(weatherLocation, hourlyData, forecast.properties.forecast, data))
        },
        minutely: {
            data:
                  ...
                  timelaps.timelaps_items
                      ...
                      .forEach((w) => {
                          acc.push({
                              ...
                              event: getWarningType(timelaps.phenomenon_id) + ' - ' + getWarningText(w.color_id),
                              description: w.color_id >= 1 ? getWarningContent(timelaps.phenomenon_id, warnings) : undefined
                          } as Alert);
                      });
                  return acc;
              }, [])
...
function getWarningType(phemononId: string) {
    switch (phemononId) {
        case '1':
            return lc('wind');
        case '2':
...
function getWarningText(colorId: number) {
    switch (colorId) {
        case 4:
...
function getWarningColor(colorId: number) {
    switch (colorId) {
        case 4:
            return 'rgb(204, 0, 0)';
        case 3:
...
function getWarningContent(phenomenonId: string, warningsResult: MFWarnings): string {
