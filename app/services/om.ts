...
export async function fetch<T = any>(apiName: string = 'forecast', queryParams: OMParams = {}) {
    return request<T>({
...
export async function getWeather(weatherLocation: WeatherLocation) {
    ...
    const forecast = await fetch<Forecast>('forecast', { latitude: coords.lat, longitude: coords.lon });
    ...
    const hourlyData = hourly.time.slice(0, hourlyIndex).map((time, index) => {
        ...
        d.icon = convertWeatherCodeToIcon(code, hourly.is_day[index]);
        d.description = weatherCodeDescription[code];
        d.temperature = Math.round(hourly.temperature_2m[index]);
        ...
        if (hourly.freezinglevel_height) {
            d.iso = hourly.freezinglevel_height[index];
        }
    ...
    const dailyLastIndex = daily.weathercode.findIndex((d) => d === null);
    const r = {
        currently: current
        ...
        daily: {
            data: daily.time.slice(0, dailyLastIndex).map((time, index) => {
                ...
                const d = {
                    ...
                    precipAccumulation: daily.precipitation_sum[index]
