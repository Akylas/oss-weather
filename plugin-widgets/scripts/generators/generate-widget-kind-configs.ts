...
function generateTypeScriptFile(configs: Map<string, WidgetKindConfig>): string {
    const entries = Array.from(configs.entries())
        .map(([kind, config]) => {
            const settingsStr = config.settings ? JSON.stringify(config.settings, null, 4).replace(/\n/g, '\n        ') : 'null';
...
function generateSwiftFile(configs: Map<string, WidgetKindConfig>): string {
    const settingsCases = Array.from(configs.entries()).map(([kind, config]) => {
        if (!config.settings) {
            return `        case "${kind}":
            return nil`;
        }
...
function main() {
    ...
    const tsPath = join(__dirname, '../../src/WidgetKindConfigs.ts');
    writeFileSync(tsPath, tsContent, 'utf-8');
    console.log(`✓ Generated TypeScript: ${tsPath}`);

    // Generate Kotlin file
    const kotlinContent = generateKotlinFile(configs);
    const kotlinPath = join(__dirname, '../../platforms/android/java/com/akylas/weather/widgets/WidgetKindConfigs.generated.kt');
