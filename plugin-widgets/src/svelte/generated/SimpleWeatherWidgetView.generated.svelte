        ...
        {:else}
            <gridlayout padding={6}>
                ...
                <gridlayout columns="*,auto">
                        <label text={data.temperature} fontSize={Math.min(size.width * 0.16, 30)} fontWeight={700} color={colorOnSurface} col={0} horizontalAlignment="center"></label>
                        <image src={`${iconService.iconSetFolderPath}/images/${data.iconPath}.png`} width={64} height={64} visibility={(data.iconPath != null) ? 'visible' : 'collapsed'} col={1} horizontalAlignment="center"></image>
                </gridlayout>
