        ...
        <gridlayout paddingLeft={10} paddingRight={10} paddingTop={6} paddingBottom={6}>
            <gridlayout padding={8} columns="auto,*,auto">
                <stacklayout orientation="vertical" col={0} verticalAlignment="center" horizontalAlignment="center">
                    <label fontSize={Math.min(size.width * 0.17, 48)} fontWeight={config.settings?.clockBold === true ? "bold" : "normal"} color={colorOnSurface} text={formatDateWithoutYear(new Date(), 'll')} horizontalAlignment="left" verticalAlignment="center"></label>
                ...
                <label text={data.locationName} fontSize={12} opacity={0.5} maxLines={1} horizontalAlignment="left" verticalAlignment="top"></label>
                <label text={data.description} fontSize={12} opacity={0.5} textAlignment="right" verticalAlignment="bottom" horizontalAlignment="right" visibility={(data.description != null) ? 'visible' : 'collapsed'}></label>
        </gridlayout>
    {:else}
        <gridlayout padding={3}>
            ...
            <gridlayout rows="*,auto">
                <label text={data.locationName} fontSize={size.width < 100 ? 8 : size.width < 150 ? 10 : 12} opacity={0.5} maxLines={1} row={1} horizontalAlignment="right" verticalAlignment="bottom"></label>
            </gridlayout>
        </gridlayout>
    {/if}
</gridlayout>
